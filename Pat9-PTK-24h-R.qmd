---
title: "Pat9 Experiment - PTK - 24H Recovery"
subtitle: "Krista Petty / Kathryn Eisenmann"
...
params:
  ...
  signal_file: "kinome_data/PTK/Pat9_KRSAsOutput_Median_SigmBg_231023124931.txt" # Change the file name to the one you have
  saturation_file: "kinome_data/PTK/Pat9_KRSAsOutput_Signal_Saturation_231023124932.txt" # Change the file name to the one you have
...
qc_data <- krsa_qc_steps(raw_data) %>%
  mutate(
    Group = str_replace_all(SampleName, " ", "_")
  ) |>
  filter(str_detect(Group, "24h_Recovery")) |> 
  mutate(
    Group = case_when(
    Group == "Pat9_DMSO_24h_Recovery" ~ "DMSO",
    Group == "Pat9_1nM_Plinabulin_24h_Recovery" ~ "PLNB-1nM",
    Group == "Pat9_10nM_Plinabulin_24h_Recovery" ~ "PLNB-10nM",
    Group == "Pat9_30nM_Plinabulin_24h_Recovery" ~ "PLNB-30nM",
  ),
    SampleName = Group
    )
...
krsa_violin_plot(data_modeled$scaled, clean_peptides, "Group", groups = groups)
```
...
krsa_violin_plot(data_modeled$scaled, clean_peptides, "Barcode", groups = groups)
```
...
krsa_heatmap(data_modeled$normalized, clean_peptides, scale = "row", groups = groups)
```
